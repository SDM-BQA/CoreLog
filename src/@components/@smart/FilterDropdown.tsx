...
export interface FilterDropdownProps<T extends string | number> {
  ...
  renderSelectedLabel?: (selected: T[]) => ReactNode;
  emptyMessage?: string;
  fullWidth?: boolean;
  ...
  return (
    <div className="relative">
      <button
        ...
        className={`${fullWidth ? "w-full" : "inline-flex"} flex items-center justify-between gap-2 px-4 py-2.5 text-sm font-medium rounded-xl border transition-all duration-200 ${
          selected.length > 0
      ...
      >
        <span className="inline-flex items-center gap-2 min-w-0">
          ...
          <span className="truncate">{renderSelectedLabel ? renderSelectedLabel(selected) : label}</span>
          {selected.length > 0 && (
        ...
        <ChevronDown size={14} className={`opacity-50 transition-transform shrink-0 ${open ? "rotate-180" : ""}`} />
      </button>

      {open && (
        <>
          <div className="fixed inset-0 z-10" onClick={() => setOpen(false)} />
          <div className={`absolute z-20 top-full mt-1.5 left-0 ${fullWidth ? "w-full" : "min-w-[160px]"} max-h-64 overflow-y-auto bg-surface border border-border rounded-lg shadow-xl py-1`}>
            {options.length === 0 ? (
